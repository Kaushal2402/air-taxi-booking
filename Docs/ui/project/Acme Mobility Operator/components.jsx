/* Acme Mobility — Operator Web Panel · shared components and icons.
   Loaded as Babel/JSX. Exposes Shell, NavRail, TopBar, Icon, etc. to window.
   The Operator Panel is an air partner's workspace: operator-scoped, with a
   persistent banner reflecting admin-controlled platform state. */
...
const ICONS = {
  ...
  route: 'M6 19a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm12-10a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm0 0v4a4 4 0 0 1-4 4H9a3 3 0 0 1-3-3V9',
  calendar: 'M4 7a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7Zm0 4h16M8 3v4m8-4v4',
  weight: 'M12 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm-5 0h10l3 14H4L7 7Z',
};
...
// ──────────────────────────────────────────────────────────────
// Operator brand lockup — the air partner's own mark. A helipad ring
// with the rotor "H", neutral and brand-agnostic. Subtitle marks this as
// the white-label Operator Console.
// ──────────────────────────────────────────────────────────────
...
function BrandLockup({ size = 'md', org = 'Helix Aviation' }) {
  const isLg = size === 'lg';
  return (
...
// ──────────────────────────────────────────────────────────────
// NavRail — left sidebar grouped by operator domain (Section 0.1)
// ──────────────────────────────────────────────────────────────
const NAV_GROUPS = [
  {
    label: 'Operations',
    items: [
      ...
      { id: 'requests',    label: 'Booking Requests', icon: 'inbox', count: '6' },
      { id: 'assignment',  label: 'Assignment',       icon: 'bolt' },
      { id: 'dayof',       label: 'Day-of-Flight',    icon: 'plane', count: '4' },
      { id: 'manifest',    label: 'Manifests',        icon: 'users' },
      { id: 'cancel',      label: 'Cancel & Reschedule', icon: 'refresh' },
    ],
  },
  {
    label: 'Fleet & Crew',
    items: [
      { id: 'aircraft', label: 'Aircraft & Fleet', icon: 'helipad' },
  ...
  },
  {
    ...
    items: [
      { id: 'routes',  label: 'Routes & Schedule', icon: 'route' },
      { id: 'pricing', label: 'Pricing & Quotes',  icon: 'tag' },
    ],
  },
  {
    label: 'Finance',
    items: [
      { id: 'payouts', label: 'Payouts & Settlements', icon: 'wallet' },
  ...
  },
  {
    label: 'Compliance',
    items: [
      { id: 'documents', label: 'Documents', icon: 'shield', count: '2' },
    ],
  },
  {
    label: 'Settings',
    items: [
      ...
      { id: 'team',          label: 'Team & Roles',    icon: 'key' },
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <span>Powered by Acme Mobility</span>
      </div>
    </aside>
  );
}
...
// ──────────────────────────────────────────────────────────────
// AdminStateBanner — persistent strip reflecting admin-controlled platform
// state (approval / publishing pause / document expiry). Read-only to the
// operator: it surfaces a state the operator cannot override.
// tone: 'ok' | 'info' | 'warn' | 'danger'
// ──────────────────────────────────────────────────────────────
function AdminStateBanner({ tone = 'info', icon = 'alert', label, message, action }) {
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 12,
      padding: '0 28px', height: 40,
      background: `var(--${tone}-soft)`,
      borderBottom: `1px solid color-mix(in oklab, var(--${tone}) 24%, var(--rule))`,
      color: 'var(--ink-2)',
      flexShrink: 0,
    }}>
      <Icon name={icon} size={15} style={{ color: `var(--${tone})`, flexShrink: 0 }} />
      <span style={{
        fontFamily: 'var(--font-mono)', fontSize: 10.5, fontWeight: 500,
        letterSpacing: '0.12em', textTransform: 'uppercase', color: `var(--${tone})`,
        whiteSpace: 'nowrap',
      }}>{label}</span>
      <span style={{ width: 1, height: 16, background: `color-mix(in oklab, var(--${tone}) 30%, var(--rule))` }} />
      <span style={{ fontSize: 12.5, color: 'var(--ink-2)', flex: 1, minWidth: 0 }}>{message}</span>
      {action}
      <span style={{
...
// ──────────────────────────────────────────────────────────────
// TopBar — operator org badge + approval state, search, bell,
// quick-create, profile (org name + signed-in role)
// ──────────────────────────────────────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 16px 0 0' }}>
        {/* operator org + approval state badge */}
        <div style={{
          ...
          <span className="dot ok" /> Helix Aviation · Approved · IN
        </div>
        <div className="input" style={{ width: 220, height: 32 }}>
          <Icon name="search" size={14} className="icon" />
          <input placeholder="Search flights, crew, requests…" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingRight: 8, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.2 }}>
            <span style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--ink)' }}>Arjun Khanna</span>
            <span style={{ fontSize: 10.5, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)', letterSpacing: '0.04em' }}>Operator Admin</span>
          </div>
...
// Optional `banner` renders the admin-controlled state strip under the topbar.
// ──────────────────────────────────────────────────────────────
function Shell({ active, title, subtitle, breadcrumb, actions, banner, children }) {
  return (
    <div style={{
      display: 'flex',
      width: '100%',
...
Object.assign(window, {
  ...
  NavRail, TopBar, Shell, AdminStateBanner,
