export interface TimezoneOption {
  value: string   // IANA zone, e.g. "Asia/Kolkata"
  label: string   // "Asia/Kolkata (IST, UTC+5:30)"
...
function getOffsetMinutes(iana: string): number {
  try {
    ...
    const localMs = new Date(new Date(now).toLocaleString('en-US', { timeZone: iana })).getTime()
...
function buildTimezones(): TimezoneOption[] {
  ...
  for (const iana of IANA_ZONES) {
    try {
      const offset = getOffsetMinutes(iana)
      const abbr   = getAbbreviation(iana)
      const city   = iana.split('/').pop()?.replace(/_/g, ' ') ?? iana
      const region = iana.split('/')[0]
      const label  = abbr
        ? `${iana.replace(/_/g, ' ')} (${abbr}, ${formatOffset(offset)})`
        : `${iana.replace(/_/g, ' ')} (${formatOffset(offset)})`
      list.push({ value: iana, label, offset, region })
    } catch {
      // skip unsupported zones in this runtime
    }
  }
  // sort by offset, then alphabetically within same offset
  list.sort((a, b) => a.offset !== b.offset ? a.offset - b.offset : a.value.localeCompare(b.value))
  return list
}

// Build once at module load — memoised
export const ALL_TIMEZONES: TimezoneOption[] = buildTimezones()

// Unique regions for optional grouping
export const TIMEZONE_REGIONS = [...new Set(ALL_TIMEZONES.map(t => t.region))]
